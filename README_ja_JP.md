...
- サーバーログ、ウェブアクセスログ、データベース操作ログ、ミドルウェアログを有効にし、一元的に収集および管理するためにsplunkのソリューションを使用できるようにしてください;
- プロジェクトのサーバーがネットワークアクセス制限を適切に設定し、IPホワイトリストアドレスとネットワークセグメントの分割を通じて、プロジェクトのネットワークアーキテクチャを最適化および標準化し、業務上必要なサーバー間のみのネットワーク通信を許可するようにしてください;
- AWS、Google Cloudなどの優れたクラウドサーバープロバイダーを選択する;
- プロジェクトに使用するクラウドプラットフォームの管理アカウントに強力なパスワードを設定し、MFA認証を有効にする;
- プロジェクトコードをサーバーにデプロイする前に、サーバーをセキュリティ強化する（例: HIDSのインストール、SSHキーを使用したログイン、SSHログインアラートの設定、SSHログインのGoogle認証設定など）;
- 専門のソフトウェアモニタリングサービスとサーバーの可用性を使用する（例: APM、Zabbixなど）;
- SlowMist、Trail of Bitsなどの専門機関によって、定期的にプロジェクトのセキュリティテストを実施する。
...
- ユニットテストのカバレッジ率が95%以上である。コアコードのカバレッジ率は100%を達成する。また、ユニットテストのカバレッジレポートを出力する。
...
- 内部セキュリティチームを育成し、新しいコードが追加されるたびに内部セキュリティチームによるコードレビューを受けさせると同時に、内部セキュリティチームの能力を継続的に向上および育成してください。参考[《SlowMist-Learning-Roadmap-for-Becoming-a-Smart-Contract-Auditor》](https://github.com/slowmist/SlowMist-Learning-Roadmap-for-Becoming-a-Smart-Contract-Auditor)。
...
- セキュリティ監査には、少なくとも3つのチームが独立して監査を行う（1つの内部チームと2つの外部チームの参加が望ましい）。
